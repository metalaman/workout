...
- Async actions call `lib/db/` domain module functions, never Appwrite directly
...
5. **Add CRUD functions** to the appropriate `lib/db/` domain module (e.g., `lib/db/body.ts` for body-related data). If creating a new domain, add a new file and re-export from `lib/db/index.ts`:
   ```typescript
...
6. **Create a Zustand store** if needed:
   ...
   import * as db from '@/lib/db'  // or '@/lib/database' (backward-compatible barrel)
...
The `program_days` collection stores exercises as a **string**, not an array. `lib/db/programs.ts` handles this automatically, but if you bypass it, you must:
```typescript
// Writing:
